subroutine initialize_para_val
use mod
...
if(allocate_err/= 0) then
    write(*,*) 'allocate para_start_speed_2arr error'
	stop
end if
...
do k=1,seed_rank
	do i=1,para_rank
    	...
    	! write(*,*) "----------------------------------------------------------------------------"
        para_start_value_3arr(k,i)%p => tmp_place_p
        para_start_speed_3arr(k,i)%p => tmp_speed_p
	end do
end do
