subroutine extract_swat_output(seed_num,simu_data_arr)
use mod
...
read_fileid = seed_num + 6
! write(*,*) 'hello world1'
open(unit=read_fileid,file=trim(simu_result_fn),status='old',&
...
! write(*,*) 'hello world2'
write_fileid = read_fileid + seed_rank + 12

! open(unit=write_fileid,file='out/'//trim(obser_val_name)//trim(strnum)//'.txt',&
! 					status='replace',action='write',iostat=open_err,iomsg=iomsg)!创建新文件文件
open(unit=write_fileid,file='out/'//trim(obser_val_name)//'.txt',&
					status='replace',action='write',iostat=open_err,iomsg=iomsg)!创建新文件文件
